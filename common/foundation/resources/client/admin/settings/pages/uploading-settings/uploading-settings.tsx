...
function useTestTelegramConnection() {
  return useMutation({
    mutationFn: (payload: object) =>
      apiClient.post<{result: TestResult}>('telegram/test', payload),
  });
}
function TelegramForm({isInvalid}: CredentialFormProps) {
  ...
  const handleTestConnection = () => {
    setTestResult(null);
    const payload = {
      storage_telegram_config_path: form.getValues(
        'client.telegram.storage_telegram_config_path',
      ),
      storage_telegram_chat_id: form.getValues(
        'client.telegram.storage_telegram_chat_id',
      ),
    };
