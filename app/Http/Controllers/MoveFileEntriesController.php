...
class MoveFileEntriesController extends BaseController
{
    ...
    public function move(SetPermissionsOnEntry $setPermissions)
    {
        ...
        $entryIds
            ->chunk(30)
            ->each(function (Collection $entryIds) use (
                $destinationId,
                $setPermissions,
                $allEntries,
                $newParent,
                ...
                $entries->each(function (FileEntry $entry) use (
                    ...
                    if ($entry->telegramFile) {
                        try {
                            // reload the entry's parent relationship to ensure path is correct
                            $entry->load('parent');
                            ...
                            \Illuminate\Support\Facades\Log::error(
                                "Could not update Telegram caption for moved file {$entry->id}. Error: " . $e->getMessage()
                            );
                        }
                    }
                });
                ...
                $allEntries->merge(
                    $entries->map(
                        fn(FileEntry $entry) => $setPermissions->execute(
                            $entry,
                        ),
                    ),
                );
